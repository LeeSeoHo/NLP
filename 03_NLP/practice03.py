...
with tf.variable_scope('prefix_encoder'):
  ...
  emb_shape = [ vocab_size, embed_size ]
  ...
  embedded = tf.nn.embedding_lookup(embeddings, seq_in)
  ...
  prefix_vectors = tf.nn.dynamic_rnn(cell, embedded, sequence_length=seq_len, initial_state=init_state, scope='rnn')[0]
...
def seq_prob(seq):
    ...
    outputs = sess.run(predictions, feed_dict={seq_in:  [ [ edge_index ] + seq_indexes ], seq_len: [ 1+len(seq) ], })[0]
